...
var initiateRedis = () => {
  const redisClient2 = process.env.NODE_ENV === "development" ? createClient({
    legacyMode: true
  }) : cloudRedis();
...
var peopleV1SchemaModifyContactGroupMembersResponseSchema = z2.object({
  canNotRemoveLastContactGroupResourceNames: z2.array(z2.string()).optional().nullable(),
...
var peopleV1SchemaPersonSchema = z2.object({
  ...
  relationshipInterests: z2.array(peopleV1SchemaRelationshipInterestSchema).optional(),
  relationshipStatuses: z2.array(peopleV1SchemaRelationshipStatusSchema).optional(),
...
var peopleV1SchemaBatchUpdateContactsResponseSchema = z2.object({
  updateResult: z2.record(peopleV1SchemaPersonResponseSchema).optional().nullable()
});
...
var createAuthClientObject = (req) => {
  ...
  function determineAuthURLStructure() {
    ...
    if (process.env.NODE_ENV === "production") {
      if (process.env.ALLOW_LOCAL_FRONTEND_WITH_CLOUD_BACKEND === "true" && ((_a = req == null ? void 0 : req.headers) == null ? void 0 : _a.referer)) {
...
var getAuthenticateClient2 = (req, res) => __async(void 0, null, function* () {
  var _a;
  try {
    ...
    if (code) {
      ...
      if (state && state !== "noSession") {
        if (((_a = req.session) == null ? void 0 : _a.hashSecret) && createHashState(req.session.hashSecret) === state) {
          if ((tokens == null ? void 0 : tokens.id_token) && (yield checkIdValidity(tokens.id_token))) {
...
var getAuthUrl = (req, res) => __async(void 0, null, function* () {
  var _a;
  try {
    const oAuth2Client = createAuthClientObject(req);
    ...
    const authorizeUrl = oAuth2Client.generateAuthUrl({
      ...
      scope: SCOPES,
      state: ((_a = req == null ? void 0 : req.body) == null ? void 0 : _a.noSession) ? "noSession" : hashState
    });
...
var checkIdValidity = (token) => __async(void 0, null, function* () {
...
var authorizeLocal = (_0) => __async(void 0, [_0], function* ({
  credentials
}) {
  if (credentials) {
    const oAuth2Client = createAuthClientObject(null);
...
var authenticateLocal = (_0) => __async(void 0, [_0], function* ({
  credentials
}) {
  try {
    if (credentials) {
...
var authorizeSession = (_0) => __async(void 0, [_0], function* ({ req }) {
  const oAuth2Client = createAuthClientObject(null);
  try {
...
var authenticateSession = (_0) => __async(void 0, [_0], function* ({ req }) {
  var _a;
  try {
    if (typeof ((_a = req.session) == null ? void 0 : _a.oAuthClient) !== "undefined") {
...
var authenticateUserSession = (req) => __async(void 0, null, function* () {
...
var authenticateUserLocal = (req) => __async(void 0, null, function* () {
  var _a;
...
var authMiddleware = (requestFunction) => (req, res) => __async(void 0, null, function* () {
  var _a;
  try {
    if ((_a = req.headers) == null ? void 0 : _a.authorization) {
      const useLocalRoute = typeof JSON.parse(req.headers.authorization) === "object";
      const auth = useLocalRoute ? yield authenticateUserLocal(req) : yield authenticateUserSession(req);
...
function remapContacts({
  results
}) {
  var _a;
  return (_a = results == null ? void 0 : results.map((contact) => {
    var _a2, _b, _c, _d, _e, _f, _g, _h, _i;
    return {
      name: Object.prototype.hasOwnProperty.call(contact.person, "names") ? (_c = (_b = (_a2 = contact == null ? void 0 : contact.person) == null ? void 0 : _a2.names) == null ? void 0 : _b[0]) == null ? void 0 : _c.displayName : (_f = (_e = (_d = contact == null ? void 0 : contact.person) == null ? void 0 : _d.emailAddresses) == null ? void 0 : _e[0]) == null ? void 0 : _f.value,
      emailAddress: (_i = (_h = (_g = contact == null ? void 0 : contact.person) == null ? void 0 : _g.emailAddresses) == null ? void 0 : _h[0]) == null ? void 0 : _i.value
    };
  })) != null ? _a : [];
}
...
var messageEncoding = ({
  ...
  const utf8Subject = subject ? `=?utf-8?B?${Buffer.from((_a = subject[0]) != null ? _a : "").toString("base64")}?=` : "";
  const messageParts = [
    `From: ${from}`,
    `To: ${to}`,
    `Cc: ${cc}`,
  ...
  return __async(this, null, function* () {
    ...
    try {
      if ("body" in req) {
        ...
        if ((response == null ? void 0 : response.status) === 200 && (response == null ? void 0 : response.data)) {
...
function parseStyleIntoObject(documentImage) {
  ...
  if (fetchedStyle.length > 0) {
    const parsedStyle = fetchedStyle.map((item) => {
      var _a;
    ...
    for (let i = 0; parsedStyle.length > i; i += 1) {
      if (parsedStyle[i]) {
        ...
        if (parts && TRACKERS_SELECTORS.some(
...
function detectAndRemove(documentImage) {
  let foundImage = null;
  if (TRACKERS_SELECTORS.filter(
    (checkValue) => documentImage.attributes.some((item) => {
      if (item.name === checkValue.attribute) {
        return item.value === checkValue.value;
      }
    }) === true
  ).length > 0 || TRACKERS_SELECTORS_INCLUDES.filter(
    (checkValueInclude) => documentImage.attributes.some((item) => {
...
function removeTrackers(orderedObject) {
  const localCopyOrderedObject = __spreadProps(__spreadValues({}, orderedObject), { removedTrackers: [] });
  ...
  $("img").each((_, documentImage) => {
    ...
    if (foundImage) {
      $(foundImage).remove();
      const srcOfTracker = documentImage == null ? void 0 : documentImage.attributes.filter(
...
var decodedResult = [];
var localGmail = null;
var inlineImageDecoder = (_0) => __async(void 0, [_0], function* ({
  attachmentData,
  messageId
}) {
  var _a, _b;
  const { body, filename, mimeType, headers } = attachmentData;
  const getAttachment2 = () => __async(void 0, null, function* () {
  ...
  if (response && typeof response !== "string" && (response == null ? void 0 : response.data)) {
    const decodedB64 = baseBase64(response.data);
    const contentID = (_b = (_a = headers == null ? void 0 : headers.find((e) => e.name === "Content-ID" || e.name === "Content-Id")) == null ? void 0 : _a.value) == null ? void 0 : _b.replace(/<|>/gi, "");
    if (contentID) {
      const attachment = {
        mimeType,
        decodedB64,
...
var loopThroughBodyParts = (_0) => __async(void 0, [_0], function* ({
  inputObject,
  signal
}) {
  ...
  const loopingFunction = (_02) => __async(void 0, [_02], function* ({
    loopObject
  }) {
    var _a, _b, _c, _d, _e;
    try {
      ...
      for (const objectKey of objectKeys) {
        if (objectKey === "body") {
          if (((_a = loopObject == null ? void 0 : loopObject.body) == null ? void 0 : _a.size) && ((_b = loopObject == null ? void 0 : loopObject.body) == null ? void 0 : _b.size) > 0) {
            if (((_c = loopObject.body) == null ? void 0 : _c.attachmentId) && localMessageId) {
            ...
            decodedString = decodeBase64(`${(_d = loopObject == null ? void 0 : loopObject.body) == null ? void 0 : _d.data}`);
        ...
        if (objectKey === "parts") {
          if ((((_e = loopObject == null ? void 0 : loopObject.body) == null ? void 0 : _e.size) === 0 || !Object.prototype.hasOwnProperty.call(loopObject, "body")) && (loopObject == null ? void 0 : loopObject.parts)) {
...
var prioritizeHTMLbodyObject = (response) => {
  ...
  if (response.emailHTML.length === 1) {
    return __spreadProps(__spreadValues({}, response), { emailHTML: response.emailHTML[0] });
  }
  if (response.emailHTML.length > 1) {
  ...
  if (htmlObject.length > 0) {
    return __spreadProps(__spreadValues({}, response), { emailHTML: htmlObject });
  }
  return __spreadProps(__spreadValues({}, response), { emailHTML: noHtmlObject });
};
var placeInlineImage = (orderedObject) => {
  if (orderedObject.emailFileHTML.length > 0) {
    ...
    const unprocessedValidObjects = orderedObject.emailFileHTML.filter((item) => !processedObjectArray.includes(item)).filter((item) => item.mimeType !== MIME_TYPE_NO_INLINE);
    return { emailFileHTML: unprocessedValidObjects, emailHTML: $.html() };
  }
  return orderedObject;
};
var bodyDecoder = (_0) => __async(void 0, [_0], function* ({
  messageId,
  inputObject,
  signal,
  gmail
...
var loopThroughParts = ({
  ...
  for (const inputParts of input) {
    ...
    if (!inputParts.parts && inputParts.filename && ((_a = inputParts == null ? void 0 : inputParts.headers) == null ? void 0 : _a.some(
      (header) => {
        var _a2;
        return (_a2 = header == null ? void 0 : header.name) == null ? void 0 : _a2.includes("Content-Disposition");
      }
    ))) {
...
function checkAttachment(message) {
  var _a;
  if ((_a = message == null ? void 0 : message.payload) == null ? void 0 : _a.parts) {
...
function findHeader(rawMessage, query) {
  ...
  if (((_a = rawMessage == null ? void 0 : rawMessage.payload) == null ? void 0 : _a.headers) && ((_c = (_b = rawMessage == null ? void 0 : rawMessage.payload) == null ? void 0 : _b.headers) == null ? void 0 : _c.find((e) => e.name === query))) {
    return (_e = (_d = rawMessage.payload.headers.find((e) => e.name === query)) == null ? void 0 : _d.value) != null ? _e : null;
  }
  if (((_f = rawMessage == null ? void 0 : rawMessage.payload) == null ? void 0 : _f.headers) && rawMessage.payload.headers.find((e) => e.name === query.toLowerCase())) {
    return (_h = (_g = rawMessage.payload.headers.find((e) => e.name === query.toLowerCase())) == null ? void 0 : _g.value) != null ? _h : null;
  }
  return null;
}
...
function handleListUnsubscribe(unsubscribeLink) {
  if (!unsubscribeLink)
    return null;
  const links = unsubscribeLink.split(",").map((link) => link.trim().replace(/(<|>)+/g, ""));
...
var remapFullMessage = (rawMessage, gmail) => __async(void 0, null, function* () {
  var _a, _b;
  return {
    id: rawMessage.id,
    ...
    payload: {
      mimeType: (_a = rawMessage == null ? void 0 : rawMessage.payload) == null ? void 0 : _a.mimeType,
      ...
      parts: (_b = rawMessage == null ? void 0 : rawMessage.payload) == null ? void 0 : _b.parts
    },
...
function threadFullRemap(rawObject, gmail) {
  return __async(this, null, function* () {
    if (rawObject.messages) {
      const mappedMessages = rawObject.messages.map(
        (message) => remapFullMessage(message, gmail)
      );
      const result = {
        id: rawObject.id,
...
var getDraft = (auth, req) => __async(void 0, null, function* () {
  ...
  try {
    ...
    if ((_a = response == null ? void 0 : response.data) == null ? void 0 : _a.message) {
      gmailV1SchemaDraftSchema.parse(response.data);
      const decodedResult2 = yield remapFullMessage(response.data.message, gmail);
...
// src/api/health.ts
var health = (req, res) => __async(void 0, null, function* () {
  try {
...
var remapSimpleMessage = (rawMessage) => __async(void 0, null, function* () {
  var _a;
  return {
    id: rawMessage.id,
    threadId: rawMessage.threadId,
...
function threadSimpleRemap(rawObject) {
  return __async(this, null, function* () {
    if (rawObject.messages) {
      const mappedMessages = rawObject.messages.map(
        (message) => remapSimpleMessage(message)
      );
      const result = {
        id: rawObject.id,
        historyId: rawObject.historyId,
...
var requestBodyCreator = (req) => {
  ...
  if (((_a = req == null ? void 0 : req.query) == null ? void 0 : _a.pageToken) && typeof req.query.pageToken === "string") {
    requestBody.pageToken = req.query.pageToken;
  }
...
var hydrateMetaList = (_0) => __async(void 0, [_0], function* ({
  gmail,
  response,
  timeStampLastFetch
}) {
...
var getSimpleThreads = (auth, req) => __async(void 0, null, function* () {
  ...
  try {
    ...
    if (!response || !(response == null ? void 0 : response.data) || response.data.resultSizeEstimate === 0) {
...
var fetchSimpleThreads = (req, res) => __async(void 0, null, function* () {
...
var restructureObject = (message) => {
  ...
  const newObject = __spreadProps(__spreadValues({}, message), { id: message.threadId });
  return newObject;
};
function handleHistoryObject({
  ...
  const toDoLabelId = (_a = storageLabels.find(
  ...
  const handleRemovalUnreadLabel = (item) => {
    ...
    if (item.labelsRemoved && item.labelsRemoved[0]) {
      if (((_a2 = item.labelsRemoved[0].labelIds) == null ? void 0 : _a2.includes(UNREAD_LABEL)) && ((_c = (_b = item.labelsRemoved[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.labelIds)) {
        const staticOnlyLegalLabels = onlyLegalLabelObjects_default({
          labelNames: item.labelsRemoved[0].message.labelIds,
          storageLabels
        });
        if (staticOnlyLegalLabels.length > 0 && staticOnlyLegalLabels.some((label) => label.id === INBOX_LABEL)) {
          inboxFeed.threads.push(
            restructureObject(item.labelsRemoved[0].message)
          );
        }
        if (staticOnlyLegalLabels.length > 0 && staticOnlyLegalLabels.some((label) => label.id === toDoLabelId)) {
  ...
  const handleRemovalOriginFeed = (item) => {
    ...
    if (item.labelsRemoved) {
      if ((_b = (_a2 = item.labelsRemoved[0]) == null ? void 0 : _a2.message) == null ? void 0 : _b.threadId) {
        const toHandleObject = item.labelsRemoved[0];
        if ((_d = (_c = item.labelsRemoved[0]) == null ? void 0 : _c.labelIds) == null ? void 0 : _d.includes(INBOX_LABEL)) {
          const output = inboxFeed.threads.filter(
            (filterItem) => {
              var _a3;
              return (filterItem == null ? void 0 : filterItem.id) !== ((_a3 = toHandleObject == null ? void 0 : toHandleObject.message) == null ? void 0 : _a3.threadId);
            }
          );
          inboxFeed.threads = output;
        }
        if ((_f = (_e = item.labelsRemoved[0]) == null ? void 0 : _e.labelIds) == null ? void 0 : _f.includes(toDoLabelId)) {
  ...
  const handleAdditionLabel = (item) => {
    var _a2, _b, _c, _d;
    if (item.labelsAdded) {
      if ((_a2 = item.labelsAdded[0].labelIds) == null ? void 0 : _a2.includes(INBOX_LABEL)) {
        inboxFeed.threads.push(restructureObject((_b = item == null ? void 0 : item.labelsAdded[0]) == null ? void 0 : _b.message));
      }
      if ((_c = item.labelsAdded[0].labelIds) == null ? void 0 : _c.includes(toDoLabelId)) {
        todoFeed.threads.push(restructureObject(item.labelsAdded[0].message));
      }
      if ((_d = item.labelsAdded[0].labelIds) == null ? void 0 : _d.includes(SENT_LABEL)) {
  ...
  const handleAdditionDraftMessage = (item) => {
    var _a2, _b, _c;
    if ((item == null ? void 0 : item.messagesAdded) && ((_a2 = item == null ? void 0 : item.messagesAdded[0]) == null ? void 0 : _a2.message) && ((_c = (_b = item == null ? void 0 : item.messagesAdded[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.labelIds)) {
      const draftThreadIndex = draftFeed.threads.findIndex((thread) => {
        var _a3;
        if (item == null ? void 0 : item.messagesAdded) {
          return (thread == null ? void 0 : thread.threadId) === ((_a3 = item.messagesAdded[0].message) == null ? void 0 : _a3.threadId);
        }
        return -1;
      });
...
var fetchHistory = (auth, req) => __async(void 0, null, function* () {
  ...
  try {
    ...
    if ((response == null ? void 0 : response.status) === 200 && storageLabels) {
...
var deleteSingleMessage = (auth, req) => __async(void 0, null, function* () {
...
var deleteMessage = (req, res) => __async(void 0, null, function* () {
...
var thrashSingleMessage = (auth, req) => __async(void 0, null, function* () {
...
var thrashMessage = (req, res) => __async(void 0, null, function* () {
...
var modifyMessage = (auth, req) => __async(void 0, null, function* () {
...
var updateMessage = (req, res) => __async(void 0, null, function* () {
...
var deleteSingleThread = (auth, req) => __async(void 0, null, function* () {
...
var deleteThread = (req, res) => __async(void 0, null, function* () {
...
var getFullThreads = (auth, req) => __async(void 0, null, function* () {
  ...
  try {
    ...
    if (response && response.data) {
      gmailV1SchemaListThreadsResponseSchema.parse(response.data);
      const hydrateMetaList2 = () => __async(void 0, null, function* () {
        const results = [];
...
var thrashSingleThread = (auth, req) => __async(void 0, null, function* () {
...
var thrashThread = (req, res) => __async(void 0, null, function* () {
...
var updateSingleThread = (auth, req) => __async(void 0, null, function* () {
...
var updateThread = (req, res) => __async(void 0, null, function* () {
...
var fetchProfile = (auth) => __async(void 0, null, function* () {
  ...
  try {
    ...
    if (userResponse.status === "fulfilled" && contactsResponse.status === "fulfilled") {
      gmailV1SchemaProfileSchema.parse(userResponse.value.data);
      peopleV1SchemaPersonSchema.parse(contactsResponse.value.data);
      const getName = () => {
        var _a, _b, _c, _d, _e;
        if (((_b = (_a = contactsResponse.value) == null ? void 0 : _a.data) == null ? void 0 : _b.names) && ((_e = (_d = (_c = contactsResponse.value) == null ? void 0 : _c.data) == null ? void 0 : _d.names) == null ? void 0 : _e.length) > 0) {
...
var fetchSendAs = (auth, req) => __async(void 0, null, function* () {
...
var getSendAs = (req, res) => __async(void 0, null, function* () {
  authMiddleware(fetchSendAs)(req, res);
});

// src/api/Users/logoutUser.ts
var logoutUser = (req, res) => __async(void 0, null, function* () {
  try {
...
var updateSendAsGmail = (auth, req) => __async(void 0, null, function* () {
...
var updateSendAs = (req, res) => __async(void 0, null, function* () {
...
// src/routes/app.ts
process.env.NODE_ENV !== "production" && console.log("Booted and ready for usage");
var app = express2();
...
app.use(
  session({
    ...
    cookie: {
      ...
      domain: process.env.NODE_ENV !== "production" ? void 0 : process.env.COOKIE_DOMAIN
    }
  })
);
...
function determineAllowOrigin(req) {
  ...
  switch (process.env.NODE_ENV) {
    case "production": {
      if (process.env.ALLOW_LOCAL_FRONTEND_WITH_CLOUD_BACKEND === "true" && ((_a = req.headers) == null ? void 0 : _a.referer)) {
        return req.headers.referer.endsWith("/") ? req.headers.referer.slice(0, -1) : req.headers.referer;
      }
...
function determineAllowCredentials(req) {
  ...
  switch (process.env.NODE_ENV) {
    case "production": {
      if (process.env.ALLOW_LOCAL_FRONTEND_WITH_CLOUD_BACKEND === "true" && ((_a = req.headers) == null ? void 0 : _a.referer) && req.headers.referer.includes("localhost")) {
        return "false";
      }
      return "true";
    }
