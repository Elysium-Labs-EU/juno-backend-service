...
var authorize = (_0) => __async(void 0, [_0], function* ({
  session: session2,
  requestAccessToken
}) {
  if (requestAccessToken && (session2 == null ? void 0 : session2.access_token) === requestAccessToken.replace(/['"]+/g, "")) {
...
var authenticate = (_0) => __async(void 0, [_0], function* ({
  session: session2,
  requestAccessToken
}) {
  try {
    if (typeof session2 !== "undefined" && requestAccessToken) {
      const response = yield authorize({ session: session2, requestAccessToken });
      return response;
    }
...
var getAuthenticateClient = (req, res) => __async(void 0, null, function* () {
  try {
    const { code } = req.body;
    if (code) {
      const oAuth2Client = createAuthClientObject();
...
var authenticateUser = (req) => __async(void 0, null, function* () {
  var _a, _b;
  const response = yield authenticate({
    session: (_a = req.session) == null ? void 0 : _a.oAuthClient,
    requestAccessToken: (_b = req.headers) == null ? void 0 : _b.authorization
  });
...
var messageEncoding = (props) => {
  ...
  const utf8Subject = `=?utf-8?B?${Buffer.from(subject != null ? subject : "").toString(
    "base64"
  )}?=`;
  const messageParts = [
    `From: ${sender}`,
...
var import_express_session = __toESM(require("./node_modules/express-session/index.js"));
var import_connect_redis = __toESM(require("./node_modules/connect-redis/index.js"));

// src/data/redis.ts
var import_redis = require("./node_modules/redis/dist/index.js");
var initiateRedis = () => {
  assertNonNullish(process.env.REDIS_PORT, "No Redis Port defined");
  const redisClient2 = process.env.NODE_ENV === "development" ? (0, import_redis.createClient)({
    legacyMode: true
...
app.use("/swagger", import_swagger_ui_express.default.serve, import_swagger_ui_express.default.setup(swaggerDocs));
