...
var initiateRedis = () => {
  const redisClient2 = process.env.NODE_ENV === "development" ? createClient() : cloudRedis();
...
}) : logtailSourceToken ? new Logtail(logtailSourceToken) : void 0;
...
var createAuthClientObject = (req) => {
  ...
  function determineAuthURLStructure() {
    if (process.env.NODE_ENV === "production") {
      if (process.env.ALLOW_LOCAL_FRONTEND_WITH_CLOUD_BACKEND === "true" && req?.headers?.referer) {
...
var getAuthenticateClient2 = async (req, res) => {
  try {
    ...
    if (state && state !== "noSession") {
      try {
        if (req.session?.hashSecret && createHashState(req.session.hashSecret) === state) {
          if (tokens && tokens?.id_token && await checkIdValidity(tokens.id_token)) {
            ...
            loggerMiddleware_default?.warn("Invalid token during client authentication");
            return res.status(401).json(global.INVALID_TOKEN);
          }
        } else {
          loggerMiddleware_default?.error("Invalid state detected during client authentication");
          res.status(401).json("Invalid state detected");
          throw new Error("Invalid state detected");
        }
      } catch (err) {
        loggerMiddleware_default?.error(`Error during state and token check: ${err}`);
        throw err;
      }
    }
    if (state === "noSession") {
    ...
    loggerMiddleware_default?.error(`Error in getAuthenticateClient function: ${err}`);
...
var authorizeLocal = async ({
  ...
  try {
    ...
    void loggerMiddleware_default?.error("Error during authorization", { error: err });
    return "Error during authorization";
  }
};
var authenticateLocal = async ({
  credentials
}) => {
  try {
    if (!credentials) {
    ...
    void loggerMiddleware_default?.error("Error during local authentication", { error: err });
  }
};
...
var authenticateSession = async ({ req }) => {
  try {
    if (typeof req.session?.oAuthClient !== "undefined") {
      ...
      void loggerMiddleware_default?.warn("Invalid session found during authentication");
    }
...
var authMiddleware = (requestFunction) => async (req) => {
  try {
    if (req.headers?.authorization) {
      ...
      const auth = useLocalRoute ? await authenticateUserLocal(req) : await authenticateUserSession(req);
...
async function createSettingsLabel(auth, presetValues) {
  ...
  const defaultSettingsString = `${SETTINGS_LABEL + SETTINGS_DELIMITER + showAvatarKeyMap.true + SETTINGS_DELIMITER + emailFetchSizeKeyMap[20] + SETTINGS_DELIMITER + showIntroductionKeyMap.true + SETTINGS_DELIMITER + flexibleFlowKeyMap.false + SETTINGS_DELIMITER + alternateActionsKeyMap.true}`;
...
var findSettings = (labels, auth) => {
  const result = labels.filter(
    (label) => label.name?.includes(`${SETTINGS_LABEL + SETTINGS_DELIMITER}`)
  );
  if (!result.length) {
    return null;
  }
  ...
  const longestSettingsLabel = result.reduce(
    (acc, curr) => {
      if (curr?.name && acc?.name && curr.name.length > acc.name.length || !acc.name) {
        return curr;
      }
      return acc;
    },
  ...
  result.filter((label) => label !== longestSettingsLabel).forEach((label) => {
    if (label && label?.id) {
...
var fetchProfile = async (auth) => {
  ...
  try {
    ...
    if (userResponse.status === "fulfilled" && contactsResponse.status === "fulfilled") {
      ...
      const getName = () => {
        const displayName = contactsResponse?.value?.data?.names?.at(0)?.displayName;
        return displayName;
      };
      const result = {
        name: getName(),
      ...
      const validatedResponse = extendedGmailV1SchemaProfileSchemaSchema.parse(result);
      return validatedResponse;
    }
  ...
  try {
    ...
    if (labelResponse.status === "rejected") {
      return res.status(500).json({
        error: `Network Error. ${labelResponse?.reason || JSON.stringify(labelResponse)}. Please try again later.`
      });
    }
    if (sendAsResponse.status === "rejected") {
      return res.status(500).json({
        error: `Network Error. ${sendAsResponse?.reason || JSON.stringify(sendAsResponse)}. Please try again later.`
      });
    }
    const promisesHaveSettledWithValues = sendAsResponse.status === "fulfilled" && labelResponse.status === "fulfilled";
    if (!promisesHaveSettledWithValues || sendAsResponse.value instanceof Error || labelResponse.value instanceof Error) {
    ...
    if (batchCreateLabels.some((createdLabel) => createdLabel instanceof Error)) {
      throw new Error("Invalid response on creation of labels");
    }
    const newlyCreatedLabels = batchCreateLabels.map((createdLabel) => {
    ...
    const labels = [.../* @__PURE__ */ new Set([...newlyCreatedLabels, ...possibleLabels])];
    ...
    const prefetchedBoxes = validatedRequestBody.map((baseLabel) => labels.find((item) => item.name === baseLabel)).filter((item) => item !== void 0);
...
function remapContacts({
  ...
  return results?.map((contact) => ({
    name: Object.prototype.hasOwnProperty.call(contact.person, "names") ? contact?.person?.names?.[0]?.displayName : contact?.person?.emailAddresses?.[0]?.value,
...
var messageEncoding = ({
  ...
  const utf8Subject = subject ? `=?utf-8?B?${Buffer.from(subject[0] ?? "").toString("base64")}?=` : "";
  const messageParts = [
    `From: ${from}`,
    `To: ${to}`,
    `Cc: ${cc}`,
...
function enhancePlainText(localString) {
  ...
  return AutoLinker.link(localString, { email: false }).replace(
    lineBreakRegex,
    "<br>"
  ) ?? "";
}
...
function parseStyleIntoObject(documentImage) {
  ...
  if (fetchedStyle.length > 0) {
    const parsedStyle = fetchedStyle.map((item) => item.value?.split(/\s*;\s*/g)).flat(1);
...
var inlineImageDecoder = async ({
  ...
  if (response && typeof response !== "string" && response?.data) {
    ...
    const contentID = headers?.find((e) => e.name === "Content-ID" || e.name === "Content-Id")?.value?.replace(/<|>/gi, "");
    if (contentID) {
      const attachment = {
        mimeType,
        decodedB64,
...
var loopThroughBodyParts = async ({
  inputObject,
  signal
}) => {
  if (signal?.aborted) {
  ...
  const loopingFunction = async ({
    ...
    try {
      ...
      for (const objectKey of objectKeys) {
        ...
        if (objectKey === "parts") {
          if ((loopObject?.body?.size === 0 || !Object.prototype.hasOwnProperty.call(loopObject, "body")) && loopObject?.parts) {
...
var loopThroughParts = ({
  ...
  for (const inputParts of input) {
    ...
    if (!inputParts.parts && inputParts.filename && inputParts?.headers?.some(
      (header) => header?.name?.includes("Content-Disposition")
    )) {
...
function findHeader(rawMessage, query) {
  if (rawMessage?.payload?.headers && rawMessage?.payload?.headers?.find((e) => e.name === query)) {
    return rawMessage.payload.headers.find((e) => e.name === query)?.value ?? null;
  }
  if (rawMessage?.payload?.headers && rawMessage.payload.headers.find((e) => e.name === query.toLowerCase())) {
    return rawMessage.payload.headers.find((e) => e.name === query.toLowerCase())?.value ?? null;
  }
  return null;
}
...
function fetchUnsubscribeLink(activeDocument) {
  ...
  $("a").each((_, documentLink) => {
    ...
    if (documentLink.childNodes.map((childNode) => {
      if (isText(childNode)) {
        return childNode.data ?? "";
      }
      return "";
    }).every((textNode) => REGEX.test(textNode)) && elementHref) {
      matchedElements.push(elementHref);
    }
    if (elementClass && REGEX.test(elementClass) && elementHref) {
      matchedElements.push(elementHref);
    }
    if (parentElement.text() && REGEX.test(parentElement.text()) && elementHref) {
      matchedElements.push(elementHref);
    }
    const previousSibling = parentElement.prev();
    if (previousSibling.length && previousSibling.find("*").toArray().every((el) => REGEX.test($(el).text())) && elementHref) {
      matchedElements.push(elementHref);
    }
  });
...
var getDraft = async (auth, req) => {
  ...
  try {
    ...
    if (response?.data?.message) {
      gmailV1SchemaDraftSchema.parse(response.data);
      const decodedResult2 = await remapFullMessage(response.data.message, gmail);
...
var getSimpleThreads = async (auth, req) => {
  ...
  try {
    ...
    if (!response || !response?.data || response.data.resultSizeEstimate === 0) {
      return {
        nextPageToken: null,
        threads: [],
        timestamp: timeStampLastFetch,
...
var handleRequestBody = ({ requestBody }) => {
  ...
  if ("isSettings" in requestBody && requestBody.isSettings && "value" in requestBody && "settingsLabel" in requestBody) {
...
var loggingMiddleware = (req, res, next) => {
  if (process.env.NODE_ENV !== "production" && loggerMiddleware_default && "add" in loggerMiddleware_default) {
    loggerMiddleware_default.defaultMeta = { ...loggerMiddleware_default.defaultMeta, headers: req.headers };
  }
  if (loggerMiddleware_default && "flush" in loggerMiddleware_default) {
...
app.use((req, res, next) => {
  if (req.session) {
    ...
    if (req.session.oAuthClient) {
      void loggerMiddleware_default?.info(`User oAuthClient: ${req.session.oAuthClient}`);
    }
  }
  next();
});
...
function determineAllowCredentials(req) {
  switch (process.env.NODE_ENV) {
    case "production": {
      if (process.env.ALLOW_LOCAL_FRONTEND_WITH_CLOUD_BACKEND === "true" && req.headers?.referer && req.headers.referer.includes("localhost")) {
        return "false";
      }
      return "true";
    }
...
app.use(
  Sentry2.Handlers.errorHandler({
    shouldHandleError(error) {
      ...
      if (error.status === 401 || error.status === 404 || error.status === 500 || isRedisError) {
        return true;
      }
      return false;
    }
