...
var authenticateLocal = async ({
  ...
  try {
    ...
    loggerMiddleware_default.error("Error during local authentication", { error: err });
  }
};
  ...
  try {
    if (typeof req.session?.oAuthClient !== "undefined") {
      ...
      if (response === INVALID_TOKEN) {
        loggerMiddleware_default.warn("Session token found to be invalid during authentication");
      }
      return response;
    } else {
      loggerMiddleware_default.warn("Invalid session found during authentication");
    }
...
var loggingMiddleware = (req, res, next) => {
  if (process.env.NODE_ENV !== "production" && "add" in loggerMiddleware_default) {
...
app.use((req, res, next) => {
  if (req.session) {
    ...
    if (req.session.oAuthClient) {
      loggerMiddleware_default.info(`User oAuthClient: ${req.session.oAuthClient}`);
    }
  }
  next();
});
