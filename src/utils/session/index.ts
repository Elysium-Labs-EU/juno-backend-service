...
import deprecate from 'npm:depd'
import onHeaders from 'npm:on-headers'
import parseUrl from 'npm:parseurl'
import { Buffer } from 'npm:safe-buffer'
import { sync as uid } from 'npm:uid-safe'
...
const debug = debugFactory('express-session')

const deprecated = deprecate('express-session')
...
function session(options: any) {
  ...
  store.generate = function (req: any) {
    req.sessionID = generateId(req)
    req.session = Session(req)
    req.session.cookie = Cookie(cookieOptions)
  ...
  return function session(req: any, res: any, next: any) {
    // self-awareness
    if (req.session) {
      next()
      return
    ...
    // inflate the session
    function inflate(req: any, sess: any) {
    ...
    function rewrapmethods(sess: any, callback: any) {
      return function () {
        if (req.session !== sess) {
          wrapmethods(req.session)
        }
    ...
    function isModified(sess: any) {
      return originalId !== sess.id || originalHash !== hash(sess)
    }

    // check if session has been saved
    function isSaved(sess: any) {
    ...
    store.get(req.sessionID, function (err: any, sess: any) {
      // error handling
...
function generateSessionId(sess: any) {
  return uid(24)
}
...
function getcookie(req: any, name: any, secrets: any) {
...
function issecure(req: any, trustProxy: any) {
...
function setcookie(res: any, name: any, val: any, secret: any, options: any) {
 ...
 * @private
 */
function unsigncookie(val: any, secrets: any) {
